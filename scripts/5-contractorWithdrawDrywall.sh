...
section "NOTE: THIS SCRIPT WILL DELIBERATELY FAIL!"
section "This script will attempt to WithdrawDrywall for the contractor"
echo "The contractor will attempt to WithdrawDrywall before they have done the WithdrawRough"
echo "So even if the InspectorNFT has been minted for DrywallPassed, the Milestones validator will reject and say that WithdrawRough must be executed first"

section "First we need to know how much the Contractor will be withdrawing."
read -p "Enter total balance (lastBalance) currently at the milestonesScript.addr before this withdrawal [1502000000]: " TOTAL_AT_SCRIPT
read -p "Enter amount Contractor wants to withdraw (this must match amount in DeployAdditional_5_ContractorWithdrawDrywall.hs [500000000]): " AMOUNT_TO_WITHDRAW
...
OLD_DATUM_FILE_NAME=4-deploy-homeowner-deposit-balance-datum.json
OLD_DATUM_FILE_FULL_PATH=$PATH_TO_MILESTONE_DEPLOY/$OLD_DATUM_FILE_NAME

NEW_DATUM_FILE_NAME=5-deploy-contractor-withdraw-drywall.json
