...
section "This script will attempt to deposit the remaining balance for the Homeowner to meet the totalCost"
...
read -p "Enter amount Homeowner wants to deposit (this must match amount in DeployAdditional_4_HomeownerDepositBalance.hs [1500000000]): " AMOUNT_TO_DEPOSIT
...
OLD_DATUM_FILE_NAME=3-deploy-contractor-first-withdraw-datum.json
OLD_DATUM_FILE_FULL_PATH=$PATH_TO_MILESTONE_DEPLOY/$OLD_DATUM_FILE_NAME

NEW_DATUM_FILE_NAME=4-deploy-homeowner-deposit-balance-datum.json
