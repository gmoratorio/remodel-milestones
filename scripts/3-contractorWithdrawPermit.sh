...
section "This script will attempt to make the first withdrawal for the Contractor"
...
read -p "Enter amount Contractor wants to withdraw (this must match amount in DeployAdditional_3_ContractorFirstWithdraw.hs [500000000]): " AMOUNT_TO_WITHDRAW
...
OLD_DATUM_FILE_NAME=2-deploy-homeowner-deposit-datum.json
OLD_DATUM_FILE_FULL_PATH=$PATH_TO_MILESTONE_DEPLOY/$OLD_DATUM_FILE_NAME

NEW_DATUM_FILE_NAME=3-deploy-contractor-first-withdraw-datum.json
NEW_DATUM_FILE_FULL_PATH=$PATH_TO_MILESTONE_DEPLOY/$NEW_DATUM_FILE_NAME

REDEEMER_FILE_NAME=redeemer-contractor-permit-payment.json
